...
_TRANSITIONS: dict[tuple[BrainStateType, BrainTrigger], BrainStateType] = {
    ...
    # P2-11 fix: WAITING_FEEDBACK waits for a real Temporal HumanGateSignal,
    # not a self-timeout. FEEDBACK_TIMEOUT is removed from the transition table.
    # Recovery is handled by the Orchestrator/Scheduler, not the state machine.
...
_ANY_SOURCE_TRIGGERS: dict[BrainTrigger, BrainStateType] = {
    BrainTrigger.UNRECOVERABLE_ERROR: BrainStateType.ABORT,
...
class BrainStateMachine:
    ...
    def can_transition(current_state: BrainStateType, trigger: BrainTrigger) -> bool:
        ...
        """
        if current_state == BrainStateType.ABORT:
            return False
        if trigger in _ANY_SOURCE_TRIGGERS:
            return True
