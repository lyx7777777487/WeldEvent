...
from src.shared.dto_context import DomainEvent
...
from src.shared.ports.gateway import EventSubscriptionConfig
...
def _make_parameter_patch(
    ...
    from src.shared.dto_decision.outputs import ParameterAdjustment
...
def _make_escalation(
    ...
    from src.shared.dto_decision.outputs import EvidenceReference
    ...
    return Escalation(
        ...
        supporting_evidence=[
            EvidenceReference(
                source="validation", reference_id="ref-1", description="Low confidence"
            )
        ],
        created_at=datetime.now(timezone.utc),
    )
...
def _make_domain_event(case_id: CaseId | None = None) -> DomainEvent:
    from src.shared.types import EventId
