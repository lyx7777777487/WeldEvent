...
Source: L1_Port_and_Contract_Design.md (Phase 4, Section 9.1).

Flow:
  1. SafetyValidator  -> if BLOCK, short-circuit to ESCALATED
...
from src.shared.ports.gateway import GatewayReadPort
...
from src.shared.types import CaseId, ValidationId
...
class ValidationPipeline(ValidationPipelinePort):
    ...
    def __init__(
        ...
        self._gateway_read = gateway_read

    async def validate(self, input_data: ValidationPipelineInput) -> ValidationPipelineOutput:
        ...
        if safety_out.result == SafetyStatus.BLOCK:
            ...
            esc_out = await self._escalation.update(
                EscalationTrackerInput(shadow_result=None)
            )
        ...
        # ---- Fetch prior memories via MemorySearchPort ----
        search_input = MemorySearchInput(
            query=MemorySearchQuery(
                case_features=context.case_data,
                feature_vector=[0.0],  # stub: no real vector in unit tests
                max_results=10,
            )
        )
        search_output = await self._memory_search.search(search_input)
        prior_memories = search_output.results

        # ---- Fetch WeldMap state via GatewayReadPort ----
        weldmap_state = await self._gateway_read.read_weldmap_snapshot(context.case_id)
        ...
        esc_out = await self._escalation.update(
            EscalationTrackerInput(shadow_result=shadow_out.result)
        )
