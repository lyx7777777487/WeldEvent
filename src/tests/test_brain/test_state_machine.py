...
class TestAbort:
    ...
    def test_abort_is_terminal_for_most_triggers(self):
        sm = _sm()
        for trigger in BrainTrigger:
            if trigger == BrainTrigger.UNRECOVERABLE_ERROR:
                continue  # UNRECOVERABLE_ERROR is the one exception
            if trigger == BrainTrigger.ABORT:
                continue  # ABORT -> ABORT is a no-op but still "valid" in any-source
    ...
    def test_unrecoverable_error_from_abort(self):
        sm = _sm()
        assert sm.can_transition(BrainStateType.ABORT, BrainTrigger.UNRECOVERABLE_ERROR) is True
        result = sm.transition(BrainStateType.ABORT, BrainTrigger.UNRECOVERABLE_ERROR)
        assert result == BrainStateType.IDLE
    ...
    def test_abort_from_abort(self):
        """ABORT trigger is any-source, so ABORT -> ABORT via .transition() works,
        but can_transition should return False (ABORT is terminal except for
        UNRECOVERABLE_ERROR)."""
        sm = _sm()
        # Direct transition call: ABORT trigger is any-source, goes to ABORT
        result = sm.transition(BrainStateType.ABORT, BrainTrigger.ABORT)
        assert result == BrainStateType.ABORT
        # But can_transition says ABORT is terminal (only UNRECOVERABLE_ERROR allowed)
        assert sm.can_transition(BrainStateType.ABORT, BrainTrigger.ABORT) is False
...
class TestUnrecoverableError:
    ...
    def test_unrecoverable_error_goes_to_idle_from_any_state(self):
        sm = _sm()
        for state in BrainStateType:
            result = sm.transition(state, BrainTrigger.UNRECOVERABLE_ERROR)
            assert result == BrainStateType.IDLE, f"From {state}"
